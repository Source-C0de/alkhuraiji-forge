...
const MENU_ITEMS = [
  ...
  { icon: LifeBuoy, label: "Support", path: "/admin/support", badgeKey: "open-tickets" },
...
function AdminLayout() {
  ...
  const openTicketCount = useSupportStore((s) => s.getOpenTicketCount());
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0A] bg-[url('/blueprint-dark.png')] bg-repeat bg-[length:64px_64px] bg-blend-soft-light relative">
        ...
        >
          ...
          <p className="text-center text-sm text-muted-foreground mb-8">
            Access the CMS and Builder Manager
          </p>
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              />
              {error && (
                <p className="text-red-500 text-xs mt-2">Incorrect password. Hint: admin123</p>
              )}
            </div>
            <button
              type="submit"
              className="w-full py-3 rounded-lg bg-gold text-black font-semibold hover:bg-gold-soft transition shadow-gold-glow"
            >
              Authenticate
            </button>
          </form>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide py-6 px-4 flex flex-col gap-2">
          {MENU_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`h-5 w-5 flex-shrink-0 transition-transform ${isActive ? "scale-110" : "group-hover:scale-110"}`}
                />
                {sidebarOpen && (
                  <span className="font-medium whitespace-nowrap flex-1">{item.label}</span>
                )}
                {"badgeKey" in item &&
                  item.badgeKey === "open-tickets" &&
                  openTicketCount > 0 &&
                  sidebarOpen && (
                    <span className="flex-shrink-0 h-5 min-w-5 px-1.5 rounded-full bg-gold text-black text-[10px] font-bold flex items-center justify-center">
                      {openTicketCount}
                    </span>
                  )}
